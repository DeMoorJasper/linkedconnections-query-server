    ...
    _read() {
        if (this.discoveryPhase) {
            ...
            this.cache.get(this.starturl).then(response => {
                ...
                const metaTriples = response.triples.filter(triple => triple.predicate === 'http://www.w3.org/ns/hydra/core#search' ||
                ...
                const searchUriTriples = metaTriples.filter(triple => triple.predicate === 'http://www.w3.org/ns/hydra/core#search' &&
                ...
                searchUriTriples.forEach(searchUriTriple => {
                    ...
                    const mappings = metaTriples.filter(triple => triple.subject === searchUri && triple.predicate === 'http://www.w3.org/ns/hydra/core#mapping');
                    ...
                    const template = N3Util.getLiteralValue(metaTriples.filter(triple =>
                        triple.subject === searchUri && triple.predicate === 'http://www.w3.org/ns/hydra/core#template')[0].object);
